...
var Game = {
    ...
    // flagPos: {x:6000, y:0} - need to fix ending
    // flagImg: undefined,

    // added to have background
    backgroundSprite: undefined,
    backgroundSpritePosition: {
        x: 0,
        y: 0
    },
    ...
    // all keyboard now, no use of mouse required. easier to restart game like this	
    ...
    // manually defining each oncoming coloured blocks
    // issue with pigeon exploding even though it hasn't hit anything visible
    // which could be something to do with the array of blocks being repeated somehow
    // once they have passed off the screen...makes little sense
    // my lecturer tried working out what it was with no success
    // right now the pigeon never explodes because of how I've manually set the course
    // changing it will bring back the problem, however the course is set

    // updates need to remedy this, make rectangle creation automatic 
    // with random gap heights (but gap always 130px or more, but less than 150px other too easy)


    // rectOdd and rectEven in pairs, one rectangle coming from top of screen, one from bottom, leaving gap of 130px usually
    rect1: {
        "id": "Rect1",
        x: 1000,
        y: 0,
    ...
    // random explosion happening here for some reason 
    // UPDATE 3/9/15 - bug gone by eliminating the big drop. having gap high there. just gone...no idea.
    rect13: {
        "id": "Rect13",
        x: 3450,
        y: 0,
    ...
    //finish line - needs a function for end of level, not just another collision as it is now
    rect25: {
        x: 6000,
        y: 0,
        w: 100,
...
// 32 = spacebar, 66 = b (go back to main menu when collision=true)
...
Game.start = function () {
    ...
    // putting in background music. thanks, Snoop
    ...
    // this doesn't work because I haven't made it
...
// added this to stop space pigeon hiding below the screen. keeps game inside the canvas
    if (Game.imgPos.y > 420) 
    {
        Game.imgPos.x = 0;
        Game.velocityY = -2.0;
        Game.onGround = true;
...
Game.draw = function () {
        ...
        if (Game.collisionStatus) {
            Game.canvasContext.drawImage(Game.fireBall, Game.imgPos.x - 5, Game.imgPos.y - 18, Game.fireBall.width,
                ...
                //Game.ST();
                //Game.movingBackground() = null;// this doesn't work - want to stop background moving
            Game.backgroundMusic.volume = 0.0; // this works - there is pause function you can use
            Game.sadMusic.play();
            Game.img = {};
            Game.imgPos = {};
