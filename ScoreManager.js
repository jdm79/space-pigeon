...
class ScoreManager {
  ...
  updateScoreDisplay() {
    ...
    if (scoreBox) {
      let html = `<div style="color: #00FF00;">Current: ${this.currentScore}</div>`;
      html += `<div style="color: #FFFF00;">High: ${this.highScore}</div>`;
      
      if (this.lastScore > 0) {
        html += `<div style="color: #FF8800;">Previous: ${this.lastScore}</div>`;
      }
